<?php

// Generated on 06/01/2014 15:07:24

// 152 out of 152 messages are translated (100%).
// 634 out of 634 words are translated (100%).

return array(
    #: classes/controller/admin/answer/appdesk.ctrl.php:30
    #: config/common/form.config.php:121
    'Answers to ‘{{title}}’' => 'Respuestas a «{{title}}»',

    #: classes/controller/admin/answer/appdesk.ctrl.php:36
    'Export (spreadsheet)' => 'Exportar (hoja de cálculo)',

    #: classes/controller/admin/answer.ctrl.php:33
    #: views/admin/answer.view.php:25
    'Answer to ‘{{title}}’' => 'Respuesta a «{{title}}»',

    #: classes/controller/admin/answer.ctrl.php:110
    'No file attached.' => 'Ningún archivo adjunto.',

    #: classes/controller/admin/form.ctrl.php:187
    #: classes/controller/admin/form.ctrl.php:202
    'Page break' => 'Salto de página',

    #: classes/controller/admin/enhancer.ctrl.php:54
    #: config/controller/admin/enhancer.config.php:76
    'Thank you. Your answer has been sent.' => 'Gracias. Se ha enviado tu respuesta.',

    #: views/admin/enhancer/blank_slate.view.php:16
    'How frustrating, you have no form to insert. But let’s not worry, shall we? Here is how it works:' => 'Vaya...aún no hay ningún formulario. No pasa nada, te explicamos como funciona:',

    #: views/admin/enhancer/blank_slate.view.php:19
    '<ul><li><a>Add your first form</a> (a new tab will open).</li>
<li>Once you’re done (it won’t take long), come back to this tab to insert your shiny new form.</li></ul>' => '<ul><li><a>Añade tu primer formulario</a> (se abre una nueva pestaña).</li>
<li>Cuando acabes (se tarda poco), vuelve a esta pestaña para insertar tu formulario recién salido del horno.</li></ul>',

    #: views/admin/enhancer/blank_slate.view.php:27
    'No forms are available in {{context}}. Go ahead, <a>add your first form in this language</a>.' => 'No hay ningún formulario en {{context}}. Anímate y <a>añade tu primer formulario en este idioma</a>.',

    #: views/admin/enhancer/blank_slate.view.php:29
    'No forms are available in {{context}}. Go ahead, <a>add your first form in this context</a>.' => 'No hay ningún formulario en {{context}}. Anímate y <a>añade tu primer formulario en este contexto</a>.',

    #: views/email.view.php:31
    'Message sent by:' => 'Mensaje enviado por:',

    #: views/admin/form/warning_answers_collected.view.php:26
    'Modifying the form may alter the collected data.' => 'Si lo modificas, puede que borres los datos recogidos.',

    #: views/admin/form/warning_answers_collected.view.php:29
    'You’re right, take me to the answers' => 'Tienes razón, volvamos a las respuestas',

    #: views/admin/form/warning_answers_collected.view.php:31
    'or' => 'o',

    #: views/admin/form/warning_answers_collected.view.php:31
    'Don’t worry, I know what I’m doing' => 'No te preocupes, sé lo que hago',

    #: views/admin/form/warning_answers_collected.view.php:49
    'Answers to this form have already been received' => 'Ya han llegado respuestas a este formulario',

    #: views/admin/fields.view.php:28
    'You have a problem here: Your Novius OS is not set up to send emails. You’ll have to ask your developer to set it up for you.' => 'Algo no funciona: no se ha configurado el envío de correos electrónicos en tu Novius OS. Pídele a tu desarrollador que te lo configure.',

    #: views/admin/fields.view.php:47
    #: views/admin/fields.view.php:92
    'Add a field' => 'Añadir un campo',

    #: views/admin/fields.view.php:53
    'Standard fields' => 'Campos estándar',

    #: views/admin/fields.view.php:54
    'Special fields' => 'Campos especiales',

    #: views/admin/fields.view.php:93
    'Add a page break' => 'Añadir un salto de página',

    #: views/admin/fields.view.php:96
    'Help us prevent spam: How much is 3 plus 8?' => 'Ayúdanos a prevenir el spam: ¿cuántos son 3 más 8?',

    #: views/admin/fields.view.php:103
    'Delete' => 'Eliminar',

    #: views/admin/fields.view.php:132
    'Form submission' => 'Envío de formularios',

    #: views/admin/fields.view.php:172
    'Are you sure?' => '¿Estás seguro/a?',

    #: views/admin/permissions.view.php:15
    'Can add, edit and delete forms and answers' => 'Puede añadir, modificar y eliminar formularios y respuestas',

    #: views/admin/permissions.view.php:22
    'Can visualise answers only' => 'Puede ver solo las respuestas',

    #: views/admin/form/popup_delete.view.php:24
    'This form has already received <strong>one answer</strong>.' => array(
        0 => 'Ya ha llegado <strong>una respuesta</strong> a este formulario.',
        1 => 'Ya han llegado <strong>{{count}} respuestas a este formulario.',
    ),

    #: views/admin/form/popup_delete.view.php:33
    'Yes, I want to delete this form and the {{count}} answers received.' => 'Sí que quiero eliminar este formulario y las {{count}} respuestas recibidas.',

    #: views/admin/form/warning_not_published.view.php:18
    'Not published' => 'Sin publicar',

    #: views/admin/form/warning_not_published.view.php:19
    'To publish this form, add it to a page, a blog post or any other text editor.' => 'Para publicar este formulario, añádelo a una página, un artículo de blog o cualquier otro editor de texto.',

    #: views/admin/form/warning_not_published.view.php:22
    '<p>Most Novius OS text editors (also called WYSIWYG editors) feature this button {{preview}} in the toolbar.</p><p>Click it to access the list of applications you can enhance your text with.</p>' => '<p>La mayoría de los editores de texto de Novius OS (también llamados editores WYSIWYG) incluyen este botón {{preview}} en la barra de herramientas.</p><p>Haz clic en él para acceder a la lista de aplicaciones con las que puedes mejorar el contenido.</p>',

    #: views/admin/form/warning_not_published.view.php:23
    'Applications' => 'Aplicaciones',

    #: views/admin/answer.view.php:15
    'Cancel' => 'Cancelar',

    #: views/admin/answer.view.php:26
    'Received on {{date}}' => 'Recibida el {{date}}',

    #: views/admin/answer.view.php:33
    'Question' => 'Pregunta',

    #: views/admin/answer.view.php:34
    'Answer' => 'Respuesta',

    #: views/admin/answer.view.php:55
    'Page {{number}}' => 'Página {{number}}',

    #: views/admin/answer.view.php:102
    #: views/admin/answer.view.php:112
    'Add to Media Centre' => 'Añadir a la Mediateca',

    #: config/permissions.config.php:11
    'Permissions for this application' => 'Permisos para esta aplicación',

    #: config/controller/admin/answer/inspector/date.config.php:18
    'Answer date' => 'Fecha de respuesta',

    #: config/controller/admin/answer/appdesk.config.php:25
    'Received on:' => 'Recibida el:',

    #. Note to translator: 'Preview' here is a label, not an action
    #: config/controller/admin/answer/appdesk.config.php:31
    'Preview' => 'Vista previa',

    #: config/controller/admin/answer/appdesk.config.php:32
    'Click on an answer to preview it.' => 'Haz clic en una respuesta para verla.',

    #: config/controller/admin/answer/appdesk.config.php:39
    'answer' => 'respuesta',

    #: config/controller/admin/answer/appdesk.config.php:40
    'answers' => 'respuestas',

    #: config/controller/admin/answer/appdesk.config.php:42
    #: config/common/answer.config.php:57
    '1 answer' => array(
        0 => '1 respuesta',
        1 => '{{count}} respuestas',
    ),

    #: config/controller/admin/answer/appdesk.config.php:46
    'Showing 1 answer out of {{y}}' => array(
        0 => 'Se muestra 1 respuesta de las {{y}}',
        1 => 'Se muestran {{x}} respuesta de las {{y}}',
    ),

    #: config/controller/admin/answer/appdesk.config.php:49
    'No answers' => 'No hay respuestas',

    #. Note to translator: This is the action that clears the 'Search' field
    #: config/controller/admin/answer/appdesk.config.php:51
    'Show all answers' => 'Mostrar todas las respuestas',

    #: config/controller/admin/form.config.php:18
    #: config/common/form.config.php:82
    'Add a form' => 'Añadir un formulario',

    #: config/controller/admin/form.config.php:47
    #: config/common/form.config.php:25
    'Title' => 'Título',

    #: config/controller/admin/form.config.php:57
    'Virtual name:' => 'Nombre virtual:',

    #: config/controller/admin/form.config.php:69
    'Captcha protected' => 'Protegido con captcha',

    #: config/controller/admin/form.config.php:77
    'Submit button’s label:' => 'Etiqueta del botón de envío:',

    #: config/controller/admin/form.config.php:80
    'I’m the submit button, click to edit me' => 'Soy el botón de envío, haz clic aquí para cambiarme el nombre',

    #: config/controller/admin/form.config.php:84
    'For every new answer, an email notification is sent to:' => 'Cada vez que hay una nueva respuesta, se envía una notificación por correo electrónico a:',

    #. Note to translator: This is a placeholder, i.e. a field’s label shown within the field
    #: config/controller/admin/form.config.php:86
    #: config/controller/admin/form.config.php:89
    'One email per line' => 'Un correo por línea',

    #: config/controller/admin/form.config.php:102
    #: config/controller/admin/form.config.php:246
    'Firstname:' => 'Nombre:',

    #: config/controller/admin/form.config.php:106
    'Lastname:' => 'Apellidos:',

    #: config/controller/admin/form.config.php:110
    'Email address:' => 'Correo electrónico:',

    #: config/controller/admin/form.config.php:118
    #: config/controller/admin/form.config.php:389
    'Single line text' => 'Texto (una línea)',

    #: config/controller/admin/form.config.php:130
    #: config/controller/admin/form.config.php:390
    'Paragraph text' => 'Texto (varias líneas)',

    #: config/controller/admin/form.config.php:142
    #: config/controller/admin/form.config.php:391
    'Multiple choice (checkboxes)' => 'Varias opciones (casillas)',

    #: config/controller/admin/form.config.php:148
    #: config/controller/admin/form.config.php:161
    'First option
Second option' => 'Primera opción
Segunda opción',

    #: config/controller/admin/form.config.php:155
    #: config/controller/admin/form.config.php:392
    'Unique choice (drop-down list)' => 'Selección única (lista desplegable)',

    #: config/controller/admin/form.config.php:168
    #: config/controller/admin/form.config.php:393
    'Unique choice (radio buttons)' => 'Selección única (botones de opción)',

    #: config/controller/admin/form.config.php:174
    'First choice
Second choice' => 'Primera opción
Segunda opción',

    #: config/controller/admin/form.config.php:181
    #: config/controller/admin/form.config.php:394
    'File' => 'Archivo',

    #: config/controller/admin/form.config.php:187
    'I’m the label of a file input, click to edit me:' => 'Soy la etiqueta de entrada de un archivo, haz clic aquí para cambiarme el nombre',

    #: config/controller/admin/form.config.php:196
    #: config/controller/admin/form.config.php:395
    'Email address' => 'Correo electrónico',

    #: config/controller/admin/form.config.php:202
    'Your email address:' => 'Tu correo electrónico:',

    #: config/controller/admin/form.config.php:209
    #: config/controller/admin/form.config.php:396
    'Number' => 'Número',

    #: config/controller/admin/form.config.php:215
    'Enter a number:' => 'Introduce un número:',

    #: config/controller/admin/form.config.php:222
    #: config/controller/admin/form.config.php:397
    'Date' => 'Fecha',

    #: config/controller/admin/form.config.php:228
    'Pick a date:' => 'Elige una fecha:',

    #: config/controller/admin/form.config.php:235
    'Full name' => 'Nombre y apellidos',

    #: config/controller/admin/form.config.php:241
    'Title:' => 'Título:',

    #: config/controller/admin/form.config.php:242
    'Ms
Mr' => '',

    #: config/controller/admin/form.config.php:250
    'Name:' => 'Nombre:',

    #: config/controller/admin/form.config.php:257
    'Address' => 'Dirección',

    #: config/controller/admin/form.config.php:263
    'First address line:' => 'Dirección (línea 1):',

    #: config/controller/admin/form.config.php:267
    'Second address line:' => 'Dirección (línea 2):',

    #: config/controller/admin/form.config.php:271
    'Postal code:' => 'Código postal:',

    #: config/controller/admin/form.config.php:275
    'City:' => 'Ciudad:',

    #: config/controller/admin/form.config.php:282
    #: config/controller/admin/form.config.php:398
    'Message' => 'Mensaje',

    #: config/controller/admin/form.config.php:288
    #: config/controller/admin/form.config.php:420
    'Message:' => 'Mensaje:',

    #: config/controller/admin/form.config.php:295
    #: config/controller/admin/form.config.php:300
    #: config/controller/admin/form.config.php:400
    'Separator' => 'Separador',

    #: config/controller/admin/form.config.php:308
    #: config/controller/admin/form.config.php:399
    'Hidden' => 'Oculto',

    #: config/controller/admin/form.config.php:315
    'I’m the label for internal use only as I won’t be shown to users:' => 'Soy una etiqueta solo para uso interno, ya que los usuarios no pueden verme:',

    #: config/controller/admin/form.config.php:322
    #: config/controller/admin/form.config.php:401
    'Variable' => '',

    #: config/controller/admin/form.config.php:342
    'Properties' => 'Propiedades',

    #: config/controller/admin/form.config.php:355
    'Further options' => 'Más opciones',

    #: config/controller/admin/form.config.php:366
    'Technical options' => 'Opciones técnicas',

    #: config/controller/admin/form.config.php:385
    'Type:' => 'Tipo:',

    #: config/controller/admin/form.config.php:410
    'Label:' => 'Etiqueta:',

    #: config/controller/admin/form.config.php:413
    'I’m the label, click to edit me:' => 'Soy la etiqueta, haz clic aquí para cambiarme el nombre',

    #: config/controller/admin/form.config.php:424
    'I’m a message, click to edit me.' => 'Soy un mensaje, haz clic aquí para cambiarme el nombre',

    #: config/controller/admin/form.config.php:431
    'Style:' => 'Estilo:',

    #: config/controller/admin/form.config.php:435
    'Standard' => 'Estándar',

    #: config/controller/admin/form.config.php:436
    'Heading 1' => 'Título 1',

    #: config/controller/admin/form.config.php:437
    'Heading 2' => 'Título 2',

    #: config/controller/admin/form.config.php:438
    'Heading 3' => 'Título 3',

    #: config/controller/admin/form.config.php:444
    'Answers:' => 'Respuestas:',

    #. Note to translator: This is a placeholder, i.e. a field’s label shown within the field
    #: config/controller/admin/form.config.php:450
    'One answer per line' => 'Une respuesta por línea',

    #: config/controller/admin/form.config.php:454
    'Mandatory' => 'Obligatorio',

    #: config/controller/admin/form.config.php:462
    'Default value:' => 'Valor por defecto:',

    #: config/controller/admin/form.config.php:469
    'Instructions for the user:' => 'Instrucciones para el usuario:',

    #: config/controller/admin/form.config.php:476
    'Width:' => 'Ancho:',

    #: config/controller/admin/form.config.php:477
    'Width: {{count}} characters' => 'Ancho: {{count}} caracteres',

    #: config/controller/admin/form.config.php:489
    'Height: {{count}} lines' => 'Alto: {{count}} líneas',

    #: config/controller/admin/form.config.php:501
    'Limited to {{count}} characters' => 'Limitado a {{count}} caracteres',

    #: config/controller/admin/form.config.php:511
    'Origin:' => 'Origen:',

    #: config/controller/admin/form.config.php:524
    'Variable name:' => 'Nombre de la variable:',

    #: config/controller/admin/form.config.php:531
    'Virtual field name:' => 'Nombre del campo virtual:',

    #: config/controller/admin/form.config.php:539
    'ID:' => 'Identificación:',

    #: config/controller/admin/form.config.php:546
    'CSS classes:' => 'Clases de CSS:',

    #: config/controller/admin/enhancer.config.php:42
    'Select a form:' => 'Seleccionar un formulario:',

    #: config/controller/admin/enhancer.config.php:48
    'Label position:' => 'Ubicación de las etiquetas:',

    #: config/controller/admin/enhancer.config.php:52
    'Top aligned' => 'Encima del campo',

    #: config/controller/admin/enhancer.config.php:53
    'Left aligned' => 'A la izquierda del campo, ajustar a la izquierda',

    #: config/controller/admin/enhancer.config.php:54
    'Right aligned' => 'A la izquierda del campo, ajustar a la derecha',

    #: config/controller/admin/enhancer.config.php:55
    'In the field (placeholder), for text fields only' => 'En el campo (placeholder), solo para campos de texto',

    #: config/controller/admin/enhancer.config.php:61
    'Once the user submitted the form:' => 'Cuando el usuario haya enviado el formulario:',

    #: config/controller/admin/enhancer.config.php:66
    'Display a message' => 'mostrar un mensaje',

    #: config/controller/admin/enhancer.config.php:67
    'Redirect to a page' => 'Llevar a una página',

    #: config/controller/admin/enhancer.config.php:100
    'Form' => 'Formulario',

    #: config/controller/admin/appdesk.config.php:17
    'form' => 'formulario',

    #: config/controller/admin/appdesk.config.php:18
    'forms' => 'formularios',

    #: config/controller/admin/appdesk.config.php:20
    #: config/common/form.config.php:76
    '1 form' => array(
        0 => '1 formulario',
        1 => '{{count}} formularios',
    ),

    #: config/controller/admin/appdesk.config.php:24
    'Showing 1 form out of {{y}}' => array(
        0 => 'Se muestra 1 formulario de {{y}}',
        1 => 'Se muestran {{x}} formularios de {{y}}',
    ),

    #: config/controller/admin/appdesk.config.php:27
    'No forms' => 'No hay formularios',

    #. Note to translator: This is the action that clears the 'Search' field
    #: config/controller/admin/appdesk.config.php:29
    'Show all forms' => 'Mostrar todos los formularios',

    #: config/model/answer.config.php:13
    'Answer received on {{date}}' => 'Respuesta recibida el {{date}}',

    #: config/common/form.config.php:28
    #: config/common/form.config.php:111
    'Answers' => 'Respuestas',

    #. Crud
    #: config/common/form.config.php:57
    'All good! Your new form has been added.' => '¡Ya está! Se ha añadido un nuevo formulario.',

    #: config/common/form.config.php:58
    'The form has been deleted.' => 'Se ha eliminado el formulario.',

    #. General errors
    #: config/common/form.config.php:61
    'This form doesn’t exist any more. It has been deleted.' => 'Este formulario ya no existe. Se ha eliminado.',

    #: config/common/form.config.php:62
    'We cannot find this form.' => 'No encontramos este formulario.',

    #. Deletion popup
    #: config/common/form.config.php:65
    'Deleting the form ‘{{title}}’' => 'Eliminar el formulario «{{title}}»',

    #: config/common/form.config.php:69
    'Yes, delete this form' => array(
        0 => 'Sí que quiero eliminar este formulario',
        1 => 'Sí que quiero eliminar estos {{count}} formularios',
    ),

    #: config/common/form.config.php:73
    'We cannot delete this form as the number of answers you’ve entered is wrong. Please amend it.' => 'No podemos eliminar este formulario ya que el número de respuestas indicado no es correcto. ¿Lo puedes corregir?',

    #: config/common/form.config.php:135
    #: config/common/form.config.php:159
    'There is no answers yet.' => 'Aún no hay respuestas.',

    #: config/common/form.config.php:141
    'Export the answers (spreadsheet)' => 'Exportar las respuestas (hoja de cálculo)',

    #: config/common/answer.config.php:23
    'Received on' => 'Recibida el:',

    #. Crud
    #: config/common/answer.config.php:41
    'The answer has been deleted.' => 'Se ha eliminado la respuesta.',

    #. General errors
    #: config/common/answer.config.php:44
    'This answer doesn’t exist any more. It has been deleted.' => 'Esta respuesta ya no existe. Se ha eliminado.',

    #: config/common/answer.config.php:45
    'We cannot find this answer.' => 'No encontramos esta respuesta.',

    #. Deletion popup
    #: config/common/answer.config.php:48
    'Deleting the answer ‘{{title}}’' => 'Eliminar la respuesta «{{title}}»',

    #: config/common/answer.config.php:52
    'Yes, delete this answer' => array(
        0 => 'Sí que quiero eliminar esta respuesta',
        1 => 'Sí que quiero eliminar estas {{count}} respuestas',
    ),

    #: config/common/answer.config.php:66
    'Visualise' => 'Ver',

    #: config/common/answer.config.php:76
    'Answer to ’{{title}}’' => 'Respuesta a «{{title}}»',

);
