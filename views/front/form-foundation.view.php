...
\Nos\Nos::main_controller()->addCss('static/apps/noviusos_form/dist/css/front/form-foundation.css');
\Nos\Nos::main_controller()->addJavascript('static/apps/noviusos_form/dist/js/front/form-foundation.min.js');
...
foreach ($fields as $name => &$field) {
    ...
    Helper_Front_Form::addAttrToThing($field['label'], 'class', $label_class);
    Helper_Front_Form::addAttrToThing($field['field'], 'class', 'input_text');

    $fieldItem = \Arr::get($field, 'item');
    if (!empty($fieldItem) && $fieldItem->getDriver()->isMandatory()) {

        // For fields using a label, add a <span> at the end
        Helper_Front_Form::addContentToThing($field['label'], ' <span class="required">*</span>');

        // For placeholder, add * at the end of placeholder's text
        if ($enhancer_args['label_position'] == 'placeholder') {
            Helper_Front_Form::addAttrToThing($field['field'], 'placeholder', ' *');
        }
    }
}
unset($field);
...
foreach ($errors as $name => $error) {
    $attrs = Helper_Front_Form::getHtmlAttrs($fields[$name]['field']);
...
$form_attrs['class'] .= ' nos-form-layout-foundation';
...
foreach ($fields as $name => $field) {
    ...
    if ($name === 'form_captcha') {
        ...
        echo Helper_Front_Form::renderTemplate($template, array(
            'label' => $field['label'],
            'field' => $field['field'],
            'instructions' => $field['instructions'],
            'label_class' => in_array($enhancer_args['label_position'], array('top', 'placeholder')) ? $widths[$available_width] : $widths[$label_width],
