...
if (!$item->is_new() && count($item->answers) > 0) {
    echo \View::forge('noviusos_form::admin/form/layout/warning_answers_collected', $view_params, false);
}

?>

<div id="<?= $uniqid = uniqid('container_') ?>" style="margin: 0 -1em;">
    <div class="line">
        <div class="col c8" style="position:relative;">
            ...
            <?= \View::forge('noviusos_form::admin/form/layout/fields_blank_slate', array(
            ...
            <div class="form_previews">
                ...
                <table class="form_preview">
                    ...
                    <tbody class="preview_container">
                        <?php
                        $page_break_count = 0;
                        foreach ($formLayout as $field_id) {
                            // Page break
                            if ($field_id === 'page_break') {
                                $page_break_count++;
                                ?>
                                <tr class="preview_row page_break">
                                    <?= \View::forge('noviusos_form::admin/form/layout/field_preview', array(
                            ...
                            // Field
                            elseif (isset($item->fields[$field_id])) {
                                ?>
                                <tr class="preview_row">
                                    <?= \View::forge('noviusos_form::admin/form/layout/field_preview', array(
                                        'content' => \Nos\Form\Service_Field::forge($item->fields[$field_id])->getPreviewHtml(),
...
require([
    ...
    'link!static/apps/noviusos_form/css/admin.css'
], function($) {
    var uniqid = '#<?= $uniqid ?>';
